...
import { Search, Loader, MessageSquare, Star, Mail, Phone, Calendar, User } from 'lucide-react';
                ...
                <div className="flex items-center gap-4 bg-brown-900 border border-gold-500/20 rounded-lg p-1">
                    {['all', 'feedback', 'contact'].map((f) => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-md text-sm transition-all capitalize ${filter === f
                                ...
                                >
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-lg ${inquiry.type === 'feedback' ? 'bg-green-900/20 text-green-400' : 'bg-blue-900/20 text-blue-400'}`}>
                                            ...
                                            <div className="flex gap-1">
                                                {[...Array(5)].map((_, i) => (
                                                    <Star
                                                        key={i}
                                                        size={16}
                                                        className={i < inquiry.rating ? "fill-gold-400 text-gold-400" : "text-gray-600"}
                                                    />
                                                ))}
                                            </div>
                                        )}
                                    ...
                                    <div className="p-4 bg-black/20 rounded-lg border border-white/5">
                                        <p className="text-cream-100 italic">
                                            "{inquiry.type === 'feedback' ? inquiry.review : inquiry.suggestions || "No additional message."}"
                                        </p>
                                    </div>
                                </motion.div>
                            ))
